...
final class HTMLDocument {
    ...
    init(data: Data) {
        ...
        _document = data.withUnsafeBytes {
            ...
            return htmlReadMemory(ptr, Int32(data.count), nil, nil, Int32(HTML_PARSE_RECOVER.rawValue |
                                                                          HTML_PARSE_NOERROR.rawValue |
                                                                          HTML_PARSE_NOWARNING.rawValue |
                                                                          HTML_PARSE_NONET.rawValue))
        }

        if let pErr = xmlGetLastError(), let messagePtr = pErr.pointee.message {
            let errorMessage = String(cString: messagePtr)
            NSLog("HTMLDocument: error - \(errorMessage)")
        }
    }
