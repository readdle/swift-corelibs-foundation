...
//            XCTAssert(matches.count == content.count && matches.count == count, "If NSString is valid path to directory then matches contain all content of directory. expected \(content) but got \(matches)")
    ...
    func test_paragraphRange() {
        ...
        // Simplest check. Whole string is one or more paragraphs.
        let wholeRange = NSRange(location: 0, length: nsText.length)
        let wholeParagraphRange = nsText.paragraphRange(for: wholeRange)
        XCTAssertEqual(wholeRange, wholeParagraphRange)
        ...
        // Every paragraph pair is checked against all possible subranges in
