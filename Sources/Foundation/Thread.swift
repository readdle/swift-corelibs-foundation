        ...
        return backtraceAddresses { (addresses, count) in
          ...
          var buffer: UnsafeMutablePointer<Int8> =
              UnsafeMutablePointer<Int8>
                  .allocate(capacity: MemoryLayout<SYMBOL_INFO>.size + 127)
          defer { buffer.deallocate() }

          let addresses: UnsafeMutableBufferPointer<PVOID?> =
              UnsafeMutableBufferPointer<PVOID?>(start: addresses, count: count)

          buffer.withMemoryRebound(to: SYMBOL_INFO.self, capacity: 1) { pSymbolInfo in
            for address in addresses {
              ...
              if SymFromAddr(hProcess, DWORD64(UInt(bitPattern: address)),
                ...
                let capacity = Int(pSymbolInfo.pointee.NameLen) + 1
                let symbol = String(unsafeUninitializedCapacity: capacity) {
                  return $0.withMemoryRebound(to: Int8.self) {
